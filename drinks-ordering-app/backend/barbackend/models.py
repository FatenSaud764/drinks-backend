...
class Order(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    # Tracks whether stock for this order's items has been deducted from inventory.
    # Logic: we deduct when moving out of 'pending' into a non-cancelled state.
    # If later cancelled we restock and set this False again.
...
class Message(models.Model):
    ...
    def __str__(self):
        return f"{self.get_message_type_display()} for Order #{self.order_id}"
    
    def get_default_content(self):
        """Returns default message content based on message type"""
        defaults = {
            'order_received': "We've got your order!",
            'preparing_order': "Your drink is being prepared.",
            'ready_for_pickup': "Your drink is ready for pickup.",
            'order_cancelled': "Unfortunately, your order was cancelled.",
            'order_delayed': "Your order will take a bit longer.",
            'new_order': "New order placed",
            'cancel_request': "Customer requested order cancellation",
        }
        return self.content or defaults.get(self.message_type, "")